 ...
 * 
 * Instructions:
 * Implement the algorithm to evaluate a POSTFIX arithmetic expression.
...
def generate_program_input():
    return [generate_postfix_expression()]

def evaluate(program_input, program_output):
    expected = evaluate_postfix(program_input[0])
    try:
